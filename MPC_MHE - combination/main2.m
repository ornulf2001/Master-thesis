...
Q_MHE=2e2*diag([10,10,10,1,1,1]); %Start out with low Q to trust measurements during start up, 
                                  %then increase Q after N_MHE+1. 
                                  %See below in loop
                                  
M_MHE=5e4*diag([10,10,10,3,3,3]);
P0 = inv(M_MHE);

Q_MPC = diag([100 10 100 3 7 3]);
R_MPC = diag([0.001 0.0001]);

Q_LQR = diag([100,100,100,5,5,5]);
