...
X0=[0.001;0.01;0.02+zeq;0;0;0;0;0;0;0;];
NT=500;
...
Q_MHE=5e2*diag([100,100,100,300,300,1,1,1,1,1]); 
...
M_MHE = 5e4*diag([5,5,5,5,5,2,2,10,2,2]);
P0 = inv(M_MHE);

Q_MPC = diag([50 50 10 10 10 1 1 1 1 1]);
...
for k=1:NT-1
    k
    if k==mhe.N_MHE+2
        mhe.Q = 5e2*mhe.Q; %This relies on having enabled dynamic update of arrival cost in MHE. 
