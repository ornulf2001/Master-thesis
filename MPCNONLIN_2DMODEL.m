...
addpath('C:\Users\mikae\OneDrive\Dokumenter\Industriell kyb høst 2024\Master Forprosjekt\MAGLEVREAL\O7\O7_simulering\Utlevering\simulator\utilities\maglev_specific')
addpath('C:\Users\mikae\OneDrive\Dokumenter\Industriell kyb høst 2024\Master Forprosjekt\MAGLEVREAL\O7\O7_simulering\Utlevering\simulator\utilities\plotting')
...
args.lbx(1:6:6*(N+1), 1) = -0.010;
args.ubx(1:6:6*(N+1), 1) = 0.010;
args.lbx(2:6:6*(N+1), 1) = -10;
args.ubx(2:6:6*(N+1), 1) = 10;
args.lbx(3:6:6*(N+1), 1) = -pi/8;
args.ubx(3:6:6*(N+1), 1) = pi/8;
args.lbx(4:6:6*(N+1), 1) = -0.1;
args.ubx(4:6:6*(N+1), 1) = -0.1;
...
t0 = 0;
x0 = [0; 0.04; 0; 0; 0; 0];    % initial point
xRef = [0; 0.0365; 0; 0; 0; 0]; % Reference point
...
while (norm((x0 - xRef), 2) > 1e-3 && it < simTime / h)
    ...
    [t0, x0, u0] = UpdateINIT(h, t0, x0, u, @maglevSystemDynamics2d, params);
...
function [t0, x0, u0] = UpdateINIT(T, t0, x0, u, f, params)
    st = x0;
    con = u(1, :)';

    % RK4 method for calculating the next state
    k1 = f(st, con, params);
    k2 = f(st + T/2*k1, con, params);
    k3 = f(st + T/2*k2, con, params);
    k4 = f(st + T*k3, con, params);
    
    x0 = st + T/6*(k1 + 2*k2 + 2*k3 + k4); % RK4 update for x0
...
function init_plot(x0)
    ...
    if isempty(MaglevPlotFig) || ~ishandle(MaglevPlotFig)
        ...
        xlim(MaglevPlotAxes, [-0.2, 0.2]);
        ylim(MaglevPlotAxes, [-0.05, 0.5]);
