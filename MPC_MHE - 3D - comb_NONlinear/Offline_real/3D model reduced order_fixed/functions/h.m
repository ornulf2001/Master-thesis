...
%     y - [3*n_y x 1] stacked magnetic field vectors at all sensors
    ...
    % Measurements from permanent magnets
    k = -params.n_w*pi*params.r_base^2;
    
    for i = 1:size(params.r_base_array,2)
        for j = 1:n_y/3
            m_vec = [0; 0; k*u(i)]; % Magnetization of solenoids (ignoring bias from permanent magnets)

            d = r_j(:,j) - params.r_base_array(:,i);         % Vector from magnet to sensor
            d_norm = norm(d);
         
            B = (mu0/(4*pi))*(3*(dot(m_vec,d))*d/d_norm^5 - m_vec/d_norm^3);
            B(3) = -B(3);

            y(3*(j-1)+1:3*j) = y(3*(j-1)+1:3*j) + B;
        end
    end
end
