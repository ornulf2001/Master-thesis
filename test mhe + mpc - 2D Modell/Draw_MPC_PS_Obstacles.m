...
for k = 1:size(xx,2)
    ...
    title(sprintf('Iteration %d of %d', k, size(xx,2)), 'FontSize', 14);
    hold off
    %figure(500)
...
% figure(1)
% subplot(211)
% stairs(t(1:find(t==max(t))),u_cl(1:find(t==max(t),1),1),'k','linewidth',1.5); axis([0 max(t) -0.75 0.75])
% ylabel('v (rad/s)')
% grid on
% subplot(212)
% stairs(t(1:find(t==max(t))),u_cl(1:find(t==max(t),1),2),'r','linewidth',1.5); axis([0 max(t) -0.85 0.85])
% xlabel('time (seconds)')
% ylabel('\omega (rad/s)')
% grid on
% 
% figure(2)
% subplot(311)
% %disp(t(1:length(xx(1,:)))')
% %disp(xx(1,1:end-1))
% 
% plot(t(1:length(xx(1,:))-1), xx(1,1:end-1), 'r', 'linewidth', 1.5); axis([0 max(t) -3 3])
% ylabel('x [m]')
% grid on
% subplot(312)
% plot(t(1:length(xx(1,:))-1),xx(2,1:end-1),'r','linewidth',1.5); axis([0 max(t) -3 3])
% ylabel('y [m]')
% grid on
% subplot(313)
% plot(t(1:length(xx(1,:))-1),xx(3,1:end-1),'r','linewidth',1.5); axis([0 max(t) -5 5])
% ylabel('theta ')
% grid on
end