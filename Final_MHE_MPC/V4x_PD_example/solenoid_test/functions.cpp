...
// Feedthrough slopes
float feedthroughSlope[NUM_SENSORS][3][4] = {{{0}}};
...
// replaces the old body of processSensorData()
void processSensorData(float currentXPos,
                       float currentXNeg,
                       float currentYPos,
                       float currentYNeg)
{
  const float currents[4] = {currentXPos, currentXNeg, currentYPos, currentYNeg};

  for (int s = 0; s < NUM_SENSORS; ++s) {
    for (int axis = 0; axis < 3; ++axis) {

      float value = rawMagField[s][axis] - meanMagField[s][axis];   // remove DC offset

      // subtract feed‑through from *all* solenoids
      for (int sol = 0; sol < 4; ++sol) {
        value += feedthroughSlope[s][axis][sol] * currents[sol];
      }

      rawMagFieldDetrended[s][axis] = value;
    }
  }
}
...
void calibrateDirectFeedthrough(){
  ...
  for (int sensorIdx = 0; sensorIdx < NUM_SENSORS; sensorIdx++) {
    for (int solenoidIdx = 0; solenoidIdx < 4; solenoidIdx++) {
      if (counts[solenoidIdx] > 0) {
        ...
        for (int axis = 0; axis < 3; ++axis) {
            float avgSensor = sumSensor[sensorIdx][axis][solenoidIdx] / counts[solenoidIdx];
            float slope = (avgSensor - meanMagField[sensorIdx][axis]) / avgCurrent;

            // store with the same sign convention as before (subtract effect in processing)
            feedthroughSlope[sensorIdx][axis][solenoidIdx] = -slope;
        }
      }
    }
  }
  ...
  for (int s = 0; s < NUM_SENSORS; ++s) {
    ...
    const char* solName[4]  = {"+X","-X","+Y","-Y"};
    for (int a = 0; a < 3; ++a) {
      Serial.print("  "); Serial.print(axisName[a]); Serial.print(": ");
      for (int sol = 0; sol < 4; ++sol) {
        Serial.print(solName[sol]); Serial.print("=");
        Serial.print(feedthroughSlope[s][a][sol], 6);
        if (sol < 3) Serial.print(", ");
      }
      Serial.println();
    }
  }
